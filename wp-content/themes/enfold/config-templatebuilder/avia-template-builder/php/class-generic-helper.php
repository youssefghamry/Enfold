...
if( ! class_exists( 'AviaHelper' ) )
{
	...
	class AviaHelper
	{
		...
		/**
		 * Check for an empty multi input field. 0 is an allowed value.
		 *
		 * @since 5.0.1
		 * @param string $multi_value
		 * @return boolean
		 */
		static public function empty_multi_input( $multi_value )
		{
			switch( trim( $multi_value ) )
			{
				case '':
				case ',':
				case ',,':
		...
		static public function multi_value_result_lockable( $multi_value, $attr_name = '', $directions = array( 'top', 'right', 'bottom', 'left' ) )
		{
			...
			foreach( $sync_input as $key => $value )
			{
				...
				if( ! $is_empty )
				{
					...
					$set_values_rules[ $attr_name . '-' . $directions[ $key ] ] = $val;
				}
			}
			...
			/**
			 * 'fill_with_0':			(string) sets all values, those not set by the user are set to 0, minified string
			 * 'set_values_only':		(string) creates rules for set elements and skips unset rules, minified string if all set, can be empty
			 * 'set_values_rules':		(array) 'set_values_only' as css rules attribute => value
			 * 'css_rules':				(string) minified string following minified CSS rules containing only set values ( e.g. margin: 15px 10px; ), can be empty
			 * 'css_rules_with_0':		(string) like 'css_rules', but containing 0 for not set values before last value ( e.g. margin: 15px 0 25px; )
			 * 'opt_values':			(string) option values as entered in string
			...
			$result = array(
						...
						'set_values_rules'	=> $set_values_rules,
						'css_rules'			=> $css_rules,
						'css_rules_with_0'	=> $css_rules_with_0,
						'opt_values'		=> $multi_value,
						'locked_opt_info'	=> array_map( 'trim', explode( ',', $locked_option_info ) ),
...
//    					'\.+$'					=> '',
		...
		static public function style_string( $atts, $key = false, $new_key = false, $append_value = '' )
		{
			...
			else //otherwise build only the styling argument
			{
				...
				{
					$split = strpos( $new_key, '{{' );
					if( false !== $split )
					{
						$attr = trim( substr( $new_key, 0, $split ) );
						$add = substr( $new_key, $split );
						$add = str_replace( array( '{', '}', 'prefix:' ), '', $add );
						$add = explode( ';', $add );

						$style_string = '';

						foreach( $add as $prefix )
						{
							$style_string .= "{$attr}: {$prefix}" . trim( $atts[ $key ] ) . "$append_value;\n";
						}

						$style_string .= "{$attr}: " . $atts[ $key ] . $append_value . '; ';
					}
					else
					{
						switch( $new_key )
		...
		{
			_deprecated_function( 'AviaHelper::css_4value_helper', '4.9.1', 'no longer needed - use callback popup template padding');
		...
		static public function av_mobile_sizes( $atts = array(), &$default = array() )
		{
			...
			$columns	= array( 'av-desktop-columns', 'av-medium-columns', 'av-small-columns', 'av-mini-columns' );
