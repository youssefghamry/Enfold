...
if( ! class_exists( 'avia_superobject' ) )
{
	class avia_superobject
	{
		 ...
		 * In that case the value of the current language is copied to all other languages unconditionally.
		...
		/**
		 * Option keys that are global option values in case of multiple languages (e.g. WPML).
		 * In that case the value of the current language is copied to all other languages conditionally when a theme option is selected,
		 * otherwise user can manage them seperatly.
		 * Option keys must be unique for all option pages (and slugs)
		 *
		 * @since 5.0
		 * @var array
		 */
		protected $global_layout_keys;
...
//			}
			 ...
			 * saved option values
			 *
			 * @since 4.8.9   assign array to avoid PHP 8.1 notice
			 */
			$database_option = get_option( $this->option_prefix, array() );
			if( ! is_array( $database_option ) )
			{
